...
int main(int argc, char **argv)
{
  ...
  for (auto sensor : sensors)
  {
    ...
    {
      sms = RsServerMediaSession::createNew(*env, sensor, device, "color" /*sensor.get_sensor_name().data()*/, "",
                                            "Session streamed by \"realsense streamer\"",
                                            True);
    }
    else
    ...
    for (auto stream_profile : sensor.getStreamProfiles())
    {
      ...
      //if (stream.format() == RS2_FORMAT_Z16 || stream.format() == RS2_FORMAT_Y8 || stream.format() == RS2_FORMAT_Y16 || stream.format() == RS2_FORMAT_RAW16 || stream.format() == RS2_FORMAT_YUYV || stream.format() == RS2_FORMAT_UYVY)
      //{
        if (stream.fps() == 6)
        {
