...
void ip_device::inject_frames_loop(std::shared_ptr<rs_rtp_stream> rtp_stream)
{
    ...
    while (rtp_stream.get()->is_enabled==true)
    {
        ...
        {			
			...
			rtp_stream.get()->frame_data_buff.timestamp = frame->m_header->metadata.timestamp;
            ...
            rtp_stream.get()->frame_data_buff.domain = frame->m_header->metadata.timestamp_domain;
